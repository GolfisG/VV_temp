%This method is responsible for Market clearing.When admin clicks,
%'Clear_Button' , this method connects in database and download all the
%appropriate data for Market's clearing.
%After our Optimal Power Flow method is converged we call another method to
%save the cleared as also the uncleared scenario of our case.
%
...
%%%%%%%%%%%%%%%%%%%%  THERMALLIMIT END  &&&&&&&&&&&&&&&&&&&&&&&&&&&    
  ...
  [q , p] = case2off(mpc_new.gen , mpc_new.gencost);
    for i =1:size(q,1)
       for j=1:size(q,2)
       if( q(i,j)<0 )
       q(i,j)=0;
   ...
   bids.P.prc = p(11:15,:);
   try 
      disp("Waiting for method Converge.....")
      
  [mpc_out, co, cb, f, dispatch, success, et] = runmarket(mpc_new, offers, bids, mkt);
  catch me
      rethrow(me)
  end
  disp(p);
  disp(q);
  if converge_error == 1
      fprintf("\n The market OPF did not converge succesfully for the data you provided.\n")
      return
  end
   saca_sql(mpc_new , mpc_out)
